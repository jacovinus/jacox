...
1. **stepbit-core Proxy**: Stepbit acts as a proxy/orchestrator, discovering tools from stepbit-core and presenting them to the user.
2. **Frontend Management**: A "Tools" view in Stepbit allows users to see registered MCP tools and their capabilities.

## Implementation Patterns

### Backend (Proxying)
Stepbit should expose an endpoint `GET /api/llm/mcp/tools` that fetches tool definitions from stepbit-core.
...
- **Caching**: Cache tool definitions on the frontend to avoid redundant API calls to stepbit-core.
